...
namespace AutoPopulatePlaylists.Plugins
{
    struct SortData
    {
        ...
        {
            string result = SortType.ToString();
            if (!IsAscending)
            {
                result += ",Desc";
            }
    ...
    class PlaylistData
    {
        ...
        public FrameType FrameType { get; set; } = FrameType.SingleColor;
        ...
        public PlaylistData(JsonNode node, bool isOld = false)
        {
            if (node != null)
            {
                if (!isOld)
                ...
                {
                    InitializeNodeOld(node);
                }
            }
        }

        void InitializeNode(JsonNode node)
        {
            if (node["Enabled"] != null)
            {
                IsEnabled = node["Enabled"].GetValue<bool>();
            }
            if (node["CategoryPanelData"] != null)
            {
                var catPanelData = node["CategoryPanelData"];
                if (catPanelData["Name"] != null)
                {
                    Name = catPanelData["Name"].GetValue<string>();
                }
                if (catPanelData["BgColor"] != null)
                {
                    var color = catPanelData["BgColor"].GetValue<string>();
                    ...
                    {
                        Logger.Log("Couldn't parse BgColor of \"" + color + "\"");
                    }
                }
                if (catPanelData["FrameType"] != null)
                {
                    var frameType = catPanelData["FrameType"].GetValue<string>();
                    ...
                    {
                        Logger.Log("Couldn't parse FrameType of \"" + frameType + "\"");
                    }
                }
                if (catPanelData["FrameColor"] != null)
                {
                    var color = catPanelData["FrameColor"].GetValue<string>();
                    if (ColorUtility.TryParseHtmlString(color, out var newColor))
                    {
                        FrameColor = newColor;
                    }
                    ...
                    {
                        Logger.Log("Couldn't parse FrameColor of \"" + color + "\"");
                    }
                }
            }
        ...
        void InitializeNodeOld(JsonNode node)
        {
            if (node["Enabled"] != null)
            {
                IsEnabled = node["Enabled"].GetValue<bool>();
            ...
            if (ColorUtility.TryParseHtmlString("#F7F1E1", out var color))
            {
                BgColor = color;
            }
            else
            {
                Logger.Log("Failed to parse HtmlString Color");
            }
        }
    }
}
